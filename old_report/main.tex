...
\begin{align*} z &= w_1 x_1 + w_2 (x_2 + x_3) + b \\   &= w_1 x_1 + (-w_1)(x_2 + x_3) + b \\   &= w_1 (x_1 - (x_2 + x_3)) + b. \end{align*}

When \(x_1 = x_2 + x_3\), this simplifies to

\[
z = b.
\]

More generally, we can find the difference between \(x_1\) and \(x_2 + x_3\) by:

\[
\left| z - \frac{b}{w_1} \right|.
\]


Hence we opted to use a SAGE model [CITATION], this is Similar to the GCN model but with an additional weight parameter for the self-loop. We also use a bias parameter. We got an accuracy of 53\%. % aber why do we give it a bias parameter?
...
\begin{figure}[H]
  \centering
  \input{assets/tree_experiment_graph.tex}
