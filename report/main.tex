...
\subsection{Global aggregating GCN (fully adjacent 2.0)}
We connect all nodes to a global node, \(x_G\), and we then let all nodes interact with this node, the edge weights are dynamic.
\begin{align}
  x^{l+1}_i &= \sigma \left(w^l_1 x_i + w^l_2 \cdot \text{AGG}\left(\{x_j: j \in \mathcal{N}(i) \} \right) + w^l_3(x_i, x_G) x_G  \right)\\
  x^{l+1}_G &= \sigma \left(w_G \cdot \text{AGG}\left(\{x_j: j \in \mathcal{V}\} \right) + b_G\right)
\end{align}
\subsection{SDRF}
