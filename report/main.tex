...
To dip our toes into Graph Neural Networks and eventually the over-squashing problem,
...
The motivation of our following experiments is that we want the aggregator to find the difference between the children nodes and the root node. Hence we want to add the neighbours to the root but subtract the root node itself. Thus, the self loop needs a separate weight parameter.

We then used a SAGE model, this is the same as the GCN model but with
