...
class GCN(nn.Module):
    ...
    def forward(self, data):
        ...
        x = self.conv2(x_prev, edge_index)

        x = self.interediate_mlp(torch.cat([x_prev, x], dim=1))
...
class SAGE(nn.Module):
    def __init__(self, normalise=False, bias=True):
        ...
        self.conv = SAGEConv(1, 1, normalize=normalise, bias=bias, aggr="sum")
        self.interediate_mlp = torch.nn.Sequential(
            torch.nn.Linear(2, 4),
            torch.nn.ReLU(),
            torch.nn.Linear(4, 1),
        )
        self.conv2 = SAGEConv(1, 1, normalize=normalise, bias=bias, aggr="sum")
